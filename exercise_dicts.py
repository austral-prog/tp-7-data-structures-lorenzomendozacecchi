...
def decrement_items(inventario, items):
    ...
    """
    #item es una lista [coal, wood, steal...]
    #inventario es diccionario ej: {coal:1}
    for item in items:
        if item in inventario:
...
def find_biggest_expense(gastos):
    ...
    if gastos == {}:
        ...
        mejor_promedio = -1
        for categoria, lista_precios in gastos.items():
...
def sum_expenses_by_type(gastos):
    ...
    """
    diccionario = {}
    for lista_tupla in gastos.values():
        for tipo, monto in lista_tupla:
            if tipo in diccionario:
                diccionario[tipo] = diccionario[tipo] + monto
            else:
                diccionario[tipo] = monto
    return diccionario
